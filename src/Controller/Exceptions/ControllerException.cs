...
namespace TorController.Exceptions
{
    public class ControllerException : TorControllerException
    {
        ...
        public ControllerException(Status status, string format, params object[] args)
            : base(format, args)
        {
            Status = status;
        }

        public ControllerException(Status status, string message, Exception innerException)
